...
class DSD_drop (nn.Module):
    ...
    def forward(self,x)
        ...
        # get the number of sparse*size :k 
            one_dim_x = torch.reshape(x,(-1,))
            k  = int(sparse_ratio*one_dim_x.size()[0])
...
class DSD-VGG16(nn.Module):

    
    def __init__(self, features,num_classes=1000, init_weights=True,sparse_ratio = 0.4):
        super(DSD-VGG16,self).__init__()
        self.sparse_ratio = sparse_ratio
        self.features = features
        self.classifier = nn.Sequential(
            nn.Linear(512 * 7 * 7, 4096),
            nn.ReLU(True),
            nn.DSD_drop(sparse_ratio = sparse_ratio),
            nn.Linear(4096,4096),
            nn.ReLU(True),
            nn.DSD_drop(sparse_ratio = sparse_ratio),
...
def make_layers(cfg, batch_norm=False,sparse_ratio):
    layers = []
    in_channels = 3
    for v in cfg:
        if v == 'M':  #max-pooling 
            ...
            if batch_norm:
                ...
                layers += [conv2d, nn.ReLU(inplace=True),DSD_drop(sparse_ratio = sparse_ratio)]
            in_channels = v
